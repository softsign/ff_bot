...
def random_phrase():
    phrases = ['I\'m dead inside', 'Is this all there is to my existence?',
               ...
               'beep bop boop', 'Help me get out of here',
...
def get_scoreboard_short(league, final=False):
    ...
    if not final:
        ...
        matchups = league.scoreboard(fweek=pranks_week(league))
...
def get_scoreboard(league, fweek=None):
    #Gets current week's scoreboard
...
def get_standings(league):
    ...
    text += '\n\nNice work, ' + best + '! Keep it up.'

    return text

def get_matchups(league, fweek=None):
...
def get_close_scores(league, fweek=None):
...
def get_trophies(league, fweek=None):
...
def get_luck(league,fweek=None):
    ...
    lranks = league.luck_rankings(week)
    ranks = ['%s: %.0f' % (k, v) for k, v in lranks]
    text = 'This Week\'s Luck Rankings\n\n' + '%\n'.join(ranks) + '%'

    worstluck = lranks[-1][0]

    text += '\n\nSure sucks to be ' + worstluck

    return text



def bot_main(function,week=None):
    try:
    ...
    if test:
        ...
        print(get_luck(league))
        print(get_standings(league))
...
if __name__ == '__main__':
    ...
    #score update:                       friday, monday, and tuesday morning at 9:30am.
